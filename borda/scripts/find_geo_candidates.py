...
def get_geo_candidates():
    ...
    MAX_SAMPLES = 500
    MIN_GENES = 1000
    MAX_CANDIDATES_RNASEQ = 1000
    MAX_CANDIDATES_MICROARRAY = 5

    # Query refinada para forzar presencia de controles normales
    term_rnaseq = '"Homo sapiens"[porgn] AND "Expression profiling by high throughput sequencing"[DataSet Type] AND (cancer OR tumor) AND (normal OR control OR healthy) AND "gse"[Entry Type]'
    df_rnaseq = fetch_by_term(term_rnaseq, 'RNA-seq', max_results=10000, max_keep=MAX_CANDIDATES_RNASEQ)
    
    term_array = '"Homo sapiens"[porgn] AND "Expression profiling by array"[DataSet Type] AND (cancer OR tumor) AND "gse"[Entry Type]'
    df_array = fetch_by_term(term_array, 'Microarray', max_results=20, max_keep=MAX_CANDIDATES_MICROARRAY)

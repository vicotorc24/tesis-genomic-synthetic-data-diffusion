...
def main():
    parser = argparse.ArgumentParser(description="Arena SOTA Élite Benchmark")
    ...
    classifiers = {
        ...
        'TabPFN': TabPFNClassifier(n_estimators=32, device=device, ignore_pretraining_limits=True),
    ...
    def run_modern_experiment(df, context_name, k=100):
        ...
        # FIX: Parseo de strings anuidos (como '[8.084E-1]') que hacían crashear a SHAP
        ...
        if len(df) > sample_size:
            print(f"   ⚖️ Optimización: Muestreando {sample_size} pacientes")
